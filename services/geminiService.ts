...
const withRetry = async <T>(
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      console.warn(`API call failed (attempt ${i + 1}/${retries + 1}):`, lastError.message);
...
const getFallbackAnalysis = (transcript: string, category: string): AIAnalysisResult => {
  return {
    suggestedTitle: `[未分析] ${category}に関するインタビュー記録 (${new Date().toLocaleDateString('ja-JP')})`,
...
export const generateNextQuestion = async (
  ...
  if (!ai || !apiKey) {
    return "ありがとうございます。詳しく教えていただけますか？（※APIキー未設定のため、AIによる自動質問生成は停止中です。設定を確認してください。）";
  }
...
`;
  ...
  try {
    return await withRetry(async () => {
      ...
      // 必須フィールドのバリデーション
      if (!result.suggestedTitle || !result.summary) {
